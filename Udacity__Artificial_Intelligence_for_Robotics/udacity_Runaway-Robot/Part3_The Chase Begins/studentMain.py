...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER = None):
    ...
    if len(OTHER[0]) > 2:
        ...
        for i in range(len(OTHER[0])-1):
            avg_dist+=distance_between(OTHER[0][i], OTHER[0][i+1])
        avg_dist=avg_dist/(len(OTHER[0])-1)
        # print(avg_dist)
        avg_turn_angle = 0
        heading = 0
        for i in range(len(OTHER[0])-2):
            ...
            x2, y2 = OTHER[0][i+1]
            x3, y3 = OTHER[0][i+2]
            dist1 = distance_between(OTHER[0][i+1], OTHER[0][i+2])
            dist2 = distance_between(OTHER[0][i], OTHER[0][i+1])
            a_x, a_y = x2-x1, y2-y1
            b_x, b_y = x3-x2, y3-y2
            avg_turn_angle+=acos((a_x*b_x + a_y*b_y)/(dist1*dist2))
            heading=atan2(b_y, b_x)
        avg_turn_angle=avg_turn_angle/(len(OTHER)-2)
