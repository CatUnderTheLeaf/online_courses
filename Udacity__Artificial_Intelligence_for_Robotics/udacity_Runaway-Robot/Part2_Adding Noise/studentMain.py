...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if len(OTHER) > 2:
        ...
        for i in range(len(OTHER)-1):
            avg_dist+=distance_between(OTHER[i], OTHER[i+1])
        avg_dist=avg_dist/(len(OTHER)-1)
        # print(avg_dist)
        avg_turn_angle = 0
        heading = 0
        for i in range(len(OTHER)-2):
            ...
            x2, y2 = OTHER[i+1]
            x3, y3 = OTHER[i+2]
            dist1 = distance_between(OTHER[i+1], OTHER[i+2])
            dist2 = distance_between(OTHER[i], OTHER[i+1])
            a_x, a_y = x2-x1, y2-y1
            b_x, b_y = x3-x2, y3-y2
