...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if not OTHER:
        OTHER = []
        # print(distance_between(measurement, OTHER))
    ...
    if len(OTHER) > 2:
        ...
        x2, y2 = OTHER[-2]
        x3, y3 = OTHER[-1]
        dist = distance_between(OTHER[-2], OTHER[-1])
        a_x, a_y = x2-x1, y2-y1
        b_x, b_y = x3-x2, y3-y2
        # a_x, a_y = x1-x2, y1-y2
        # b_x, b_y = x2-x3, y2-y3
        ox_x, ox_y = 1, 0
        turn_angle = acos((a_x*b_x + a_y*b_y)/(a_x**2 + a_y**2))
        # heading = acos((x3-x2)/dist)-(len(OTHER))*turn_angle
        # heading = acos((b_x*ox_x + b_y*ox_y)/(dist*1))
        heading = atan2(b_y, b_x)
        x_ = x3 + dist*cos(angle_trunc(heading+turn_angle))
        y_ = y3 + dist*sin(angle_trunc(heading+turn_angle))
...
test_target = robot(2.1, 4.3, -2.5, 2*pi / 34.0, 1.5)
